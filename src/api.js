...
export function applyLivePriceUpdates() {
  ...
  document.querySelectorAll('[data-sym]').forEach(function (el) {
    ...
    if (nd && nd !== 'loading' && nd !== 'error') {
      ...
      if (spans[1].textContent !== newNat) { spans[1].textContent = newNat; spans[1].className = 'stat-val ' + (v < 1 ? 'hot' : v < 2.0 ? 'warn' : 'up'); }
    }
    ...
    if (fvCoin && fvStatsEl) {
      ...
      if (fvSpans.length >= 3) {
        ...
        if (fvNd && fvNd !== 'loading' && fvNd !== 'error') {
          ...
          if (fvSpans[1].textContent !== newFvNat) { fvSpans[1].textContent = newFvNat; fvSpans[1].className = 'stat-val ' + (fvV < 1 ? 'hot' : fvV < 2.0 ? 'warn' : 'up'); }
        }
  ...
  var briefingText = Object.keys(byDate).sort().reverse().map(function (date) {
    return date + ':\n' + byDate[date].map(function (e) {
      ...
      var pnlStr = (t && t.status === 'ok' && t.count > 0)
        ...
        : ' | нет сделок';
      var statusLabels = { watching: 'наблюдение', traded: 'торговал', skip: 'пропуск', missed: 'упустил' };
