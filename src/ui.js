...
function _topbarHTML() {
  var onScreener = window.location.hash === '#/screener';
  return '<div class="topbar"><div class="filters">'
    + '<button class="topbar-logo" data-action="refresh" title="Обновить">' + _LOGO_SVG + '</button>'
    + '<button class="tier-badge" data-action="cycle-tier">Tear</button>'
    + '<button class="nav-pill' + (onScreener ? ' active' : '') + '" data-action="go-screener">Screener</button>'
...
export function renderScreener() {
  ...
  var rows = top6.map(function (c, i) {
    var chg = c.price_change_percentage_24h || 0;
    var chgStr = (chg >= 0 ? '+' : '') + chg.toFixed(2) + '%';
    var price = fmtPrice(c.current_price);
    var vol = fmt(c.total_volume);
    return '<div class="scr-row" data-action="expand" data-sym="' + c.symbol + '">'
      + '<span class="scr-rank">' + (i + 1) + '</span>'
      + '<span class="scr-sym">' + c.symbol.toUpperCase() + '</span>'
      + '<span class="scr-chg up">' + chgStr + '</span>'
      + '<span class="scr-price">' + price + '</span>'
      + '<span class="scr-vol">' + vol + '</span>'
      + '</div>';
  }).join('');

  var body = state.coins.length === 0
    ? '<div class="scr-empty">Загрузка данных...</div>'
    : rows;

  app.innerHTML = _topbarHTML()
    + '<div class="scr-wrap"><div class="scr-header">'
    + '<span class="scr-h-rank">#</span>'
    + '<span class="scr-h-sym">Монета</span>'
    + '<span class="scr-h-chg">Рост 24ч</span>'
    + '<span class="scr-h-price">Цена</span>'
    + '<span class="scr-h-vol">Объём</span>'
    + '</div>'
    + body
    + '</div>';
}
