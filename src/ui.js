...
export function renderJournalChart(container, history) {
  ...
  var chart = window.LightweightCharts.createChart(container, {
    ...
    timeScale: { borderColor: c.border, timeVisible: false, rightOffset: 0, fixLeftEdge: true, fixRightEdge: true, tickMarkFormatter: function (t, type) {
...
function _initChartForSym(sym) {
  ...
  if (!state.chartTF[sym]) state.chartTF[sym] = state.globalTF || '5m';
  fetchChart(sym, state.chartTF[sym]);
}
...
function _sessionWheelSVG(percent) {
  ...
  return '<svg class="session-wheel" width="14" height="14" viewBox="0 0 14 14">'
    + '<circle cx="7" cy="7" r="5" fill="none" stroke="var(--steel)" stroke-width="3"/>'
    + '<circle cx="7" cy="7" r="5" fill="none" stroke="var(--bullish)" stroke-width="3" '
...
export function injectDemoBanner() {
  ...
  el.innerHTML = '<span class="demo-banner-text">Sync watchlist · Price alerts · Track PnL · Journal · AI analysis</span>'
