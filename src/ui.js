...
function natrDisplay(symbol) {
  ...
  var v = nd.value, cls = v < 1 ? 'hot' : v < 2.0 ? 'warn' : 'up';
...
function drawRuler(sym, p1, p2, pr1, pr2) {
  ...
  if (lbl) {
    ...
    lbl.innerHTML =
      '<div style="display:flex;height:14px;align-items:center"><span style="min-width:.55em;text-align:right">' + sign + '</span><span>' + digits + '</span></div>' +
      (durStr ? '<div style="display:flex;height:14px;align-items:center"><span style="min-width:.55em"></span><span>' + durStr + '</span></div>' : '');
...
function _initChartForSym(sym) {
  ...
  lbl.style.cssText = 'position:absolute;pointer-events:none;z-index:6;display:none;white-space:nowrap;font:500 10px/10px Manrope,Arial,sans-serif;';
  el.appendChild(lbl);
...
export function openCoinFullView(sym) {
  ...
  fvLblEl.style.cssText = 'position:absolute;pointer-events:none;z-index:6;display:none;white-space:nowrap;font:500 10px/10px Manrope,Arial,sans-serif;';
